...
ServerNode::SharedPtr ServerNode::make(const ServerNodeConfig &_config,
  ...
  if (!server_node->is_ready()) {
    RCLCPP_ERROR(server_node->get_logger(), "unable to initialize parameters.");
    return nullptr;
  }
  server_node->print_config();

  // Starting the machine fleet server
  ServerConfig server_config = server_node->server_node_config.get_server_config();
  Server::SharedPtr server = Server::make(server_config);
  if (!server)
    return nullptr;

  server_node->start(Fields{std::move(server)});
...
ServerNode::ServerNode(const ServerNodeConfig &_config, const rclcpp::NodeOptions &_node_options)
    : Node(_config.fleet_name + "_node", _node_options), server_node_config(_config) {}

void ServerNode::print_config() { server_node_config.print_config(); }

void ServerNode::setup_config() {
  get_parameter("fleet_name", server_node_config.fleet_name);
  get_parameter("machine_names", server_node_config.machine_names);
...
void ServerNode::start(Fields _fields) {
  ...
  update_state_callback_group = create_callback_group(rclcpp::CallbackGroupType::MutuallyExclusive);

  update_state_timer = create_wall_timer(100ms, std::bind(&ServerNode::update_state_callback, this),
  ...
  fleet_state_pub = create_publisher<machine_fleet_msgs::msg::FleetMachineState>(
  ...
  fleet_state_pub_timer = create_wall_timer(
      ...
      std::bind(&ServerNode::publish_fleet_state, this), fleet_state_pub_callback_group);
...
bool ServerNode::is_request_valid(const std::string &_machine_name) {
...
void ServerNode::handle_machine_request(machine_fleet_msgs::msg::MachineRequest::UniquePtr _msg) {
  messages::MachineRequest mf_msg;
  to_mf_message(*(_msg.get()), mf_msg);
  fields.server->send_machine_request(mf_msg);
}

void ServerNode::handle_station_request(machine_fleet_msgs::msg::StationRequest::UniquePtr _msg) {
...
void ServerNode::update_state_callback() {
  ...
  for (const messages::MachineState &mf_ms : new_machine_states) {
    ...
    if (it == machine_states.end())
      RCLCPP_INFO(get_logger(), "registered a new machine: [%s]", ros_ms.machine_name.c_str());
...
void ServerNode::publish_fleet_state() {
  ...
  for (const auto &it : machine_states) {
    ...
    rmf_frame_ms.dispenser_mode = fleet_frame_ms.dispenser_mode;
    rmf_frame_ms.dispenser_request_id = fleet_frame_ms.dispenser_request_id;
    rmf_frame_ms.request_dropoff = fleet_frame_ms.request_dropoff;
    rmf_frame_ms.ingestor_mode = fleet_frame_ms.ingestor_mode;
    rmf_frame_ms.ingestor_request_id = fleet_frame_ms.ingestor_request_id;
    rmf_frame_ms.station_states = fleet_frame_ms.station_states;
