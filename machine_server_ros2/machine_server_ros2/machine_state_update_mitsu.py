...
class MachineStateUpdate(Node):
    ...
    def station_request_callback(self, msg: StationRequest):
        ...
        if station is not None:
            ...
            if msg.mode == StationRequest.MODE_EMPTY:
                self.pyPLC.batchwrite_bitunits(station.headdevice, [0])
            else:
                self.pyPLC.batchwrite_bitunits(station.headdevice, [1])
        else:
    ...
    def timer_callback(self):
        ...
        if self.mode_operation == "combine":
            ...
            requestDelivery = self.pyPLC.batchread_bitunits(self.request_delivery_bit, 2)
        ...
        if self.stations_quantity > 0:
            stationData = self.pyPLC.batchread_bitunits(
