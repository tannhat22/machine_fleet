...
from machine_fleet_msgs.msg import StationMode
...
def main(argv = sys.argv):
    ...
    default_topic_name = '/station_request'
    ...
    rclpy.init()
    node = rclpy.create_node('send_station_request_node')
    pub = node.create_publisher(StationRequest, args.topic_name, 10)
    ...
    if args.mode == 'mode':
        print('Please insert desired mode: empty or filled')
        return
    elif args.mode == 'empty':
        msg.mode.mode = StationMode.MODE_EMPTY 
    elif args.mode == 'filled':
        msg.mode.mode = StationMode.MODE_FILLED
    else:
        print('unrecognized mode requested, only use empty or filled please')
        return
