...
Client::SharedPtr Client::make(const ClientConfig &_config) {
  ...
  dds::DDSPublishHandler<MachineFleetData_MachineState>::SharedPtr state_pub(
      new dds::DDSPublishHandler<MachineFleetData_MachineState>(
          ...
          _config.get_namespaced_topic(_config.dds_state_topic)));

  dds::DDSSubscribeHandler<MachineFleetData_MachineRequest>::SharedPtr machine_request_sub(
      new dds::DDSSubscribeHandler<MachineFleetData_MachineRequest>(
          participant, &MachineFleetData_MachineRequest_desc,
          _config.get_namespaced_topic(_config.dds_machine_request_topic)));

  dds::DDSSubscribeHandler<MachineFleetData_StationRequest>::SharedPtr station_request_sub(
      new dds::DDSSubscribeHandler<MachineFleetData_StationRequest>(
          participant, &MachineFleetData_StationRequest_desc,
          _config.get_namespaced_topic(_config.dds_station_request_topic)));

  if (!state_pub->is_ready() || !machine_request_sub->is_ready() ||
      !station_request_sub->is_ready())
    return nullptr;

  client->impl->start(ClientImpl::Fields{std::move(participant), std::move(state_pub),
                                         std::move(machine_request_sub),
                                         std::move(station_request_sub)});
  return client;
}

Client::Client(const ClientConfig &_config) { impl.reset(new ClientImpl(_config)); }
